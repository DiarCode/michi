...
def get_crowding_predictions(db: Session = Depends(get_db_session)):
    ...
    for station in stations:
        ...
        for fc in forecasts[:4]:  # 4 horizons: 15, 30, 60, 120 min
            ...
            level = "low" if fc.predicted < base * 0.6 else "medium" if fc.predicted < base * 1.2 else "high" if fc.predicted < base * 1.8 else "very_high"
...
def get_service_changes(db: Session = Depends(get_db_session)):
    ...
    ).order_by(AlertORM.created_at.desc()).limit(10).all()
