...
To demonstrate generalisation beyond the synthetic Astana network, we also evaluate DTS-GSSF on a publicly available open dataset from the Los Angeles County Metropolitan Transportation Authority (LACMTA) Open Data Portal. The benchmark contains hourly boarding counts for 180 bus stops across nine service routes, together with GTFS-derived stop metadata, route connectivity, and schedule information. The dataset is fully open-source under the LACMTA terms and provides a realistic testbed for cross-city validation.
