...
export const fetchStations = (
  ...
  api
    .get("/stations", { params: { limit: 2000, ...(hour !== undefined ? { hour } : {}) } })
    .then((r) => r.data)
...
export const fetchRouteStops = (
  routeId: string
): Promise<{ route_id: string; stops: RouteStop[] }> =>
...
export const fetchOperationsReport = (
  format?: string
): Promise<OperationsReport> =>
  api
    .get("/dashboard/operations", { params: format ? { format } : {} })
    .then((r) => r.data)
...
export const startTraining = (
  ...
  api
    .post("/analytics/start", null, { params: epochs ? { epochs } : {} })
    .then((r) => r.data)

export const uploadRidership = (
  file: File
): Promise<{ status: string; rows_received: number; filename: string }> => {
...
): Promise<{ predictions: PredictionPoint[] }> =>
  api
    .get("/analytics/predictions", {
      params: horizonMinutes ? { horizon_minutes: horizonMinutes } : {},
    })
    .then((r) => r.data)
...
export const fetchInterventions = (
  ...
  api
    .get("/interventions", { params: status ? { status } : {} })
    .then((r) => r.data)
...
export const updateInterventionStatus = (
  ...
  api
    .patch(`/interventions/${id}`, null, {
      params: { status, approved_by: approvedBy },
    })
    .then((r) => r.data)
...
export const fetchExecutiveTrends = (days?: number): Promise<AnalyticsTrends> =>
  api
    .get("/executive/trends", { params: days ? { days } : {} })
    .then((r) => r.data)

export const fetchROISummary = (): Promise<ROISummary> =>
...
export const fetchDepotRecommendations = (
  depotId: string
): Promise<{ depot_id: string; recommendations: unknown[] }> =>
...
export const fetchServiceChanges = (): Promise<{
...
// Simulation
export const startSimulation = (): Promise<{
  status: string
  task_id?: string
}> => api.post("/simulation/start").then((r) => r.data)

export const stopSimulation = (): Promise<{
  status: string
  task_id?: string
...
export const fetchSimulationMetrics = (
  hoursBack?: number
): Promise<{
  realtime: {
    tick: number
  ...
  api
    .get("/simulation/metrics", {
      params: hoursBack ? { hours_back: hoursBack } : {},
    })
    .then((r) => r.data)

// Weather
export const fetchWeatherCurrent = (): Promise<WeatherReading> =>
  api.get("/weather/current").then((r) => r.data)

export const fetchWeatherForecast = (
  hours: number = 24
): Promise<{ forecast: WeatherReading[]; hours: number }> =>
  api.get("/weather/forecast", { params: { hours } }).then((r) => r.data)

export const fetchWeatherImpact = (): Promise<WeatherImpact> =>
  api.get("/weather/impact").then((r) => r.data)

export const fetchWeatherHistory = (
  hours: number = 24
): Promise<{ history: WeatherReading[]; hours: number }> =>
  api.get("/weather/history", { params: { hours } }).then((r) => r.data)
