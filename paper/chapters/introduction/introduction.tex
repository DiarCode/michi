...
\begin{itemize}
    \item We present DTS-GSSF (Dual-Timescale Graph Gated Forecasting), a hybrid architecture that unifies a gated recurrent temporal encoder, adaptive graph propagation, and multi-head temporal attention for passenger flow prediction.
    ...
    \item We evaluate DTS-GSSF on a synthetic but realistic dataset of 1.3 million ridership records across 374 Astana bus stations, achieving $R^2 = 0.978 \pm 0.001$ and MAE $= 11.65 \pm 0.14$ passengers per station-hour (all 42 series), outperforming all baseline methods.
\end{itemize}
