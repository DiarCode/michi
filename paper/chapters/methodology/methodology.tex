...
This chapter presents the complete methodology of the study, including the problem formulation, dataset design and generation, the DTS-GSSF architecture, feature engineering, training strategy, and system workflow. Each component is described in sufficient detail to permit reproduction and extension of this work.
...
Given a historical context window of $T = 72$ hours, the passenger boarding count $y_{t+h,i}$ at each station $i$ for horizons $h \in \{1, 2, 3, 4\}$ corresponding to 15, 30, 60, and 120 minutes is the target quantity. Formally, the input is a tensor $\mathbf{X} \in \mathbb{R}^{B \times T \times N \times F}$ and the output is a set of Negative Binomial parameters $(\mu_{h,i}, \kappa)$ for each horizon $h$ and station $i$. The prediction task is:
\begin{equation}
...
\midrule
$\mathcal{G}$ & Graph & $|\mathcal{V}|, |\mathcal{E}|$ & Bus network graph $\mathcal{G} = (\mathcal{V}, \mathcal{E})$ \\
$N$ & Scalar & $1$ & Number of bus stations ($N = 374$) \\
$T$ & Scalar & $1$ & Context window length in hours ($T = 72$) \\
$F$ & Scalar & $1$ & Number of input features ($F = 16$) \\
$H_{\text{pred}}$ & Scalar & $1$ & Number of prediction horizons ($H_{\text{pred}} = 4$) \\
$\mathbf{X}$ & Tensor & $\mathbb{R}^{B \times T \times N \times F}$ & Input feature tensor \\
$\mathbf{x}_{t,i}$ & Vector & $\mathbb{R}^{F}$ & Feature vector for station $i$ at time $t$ \\
$\mathbf{s}_t$ & Matrix & $\mathbb{R}^{N \times d_{\text{model}}}$ & GRE hidden state at time $t$ \\
$\mathbf{u}_t$ & Matrix & $\mathbb{R}^{N \times d_{\text{model}}}$ & Projected GRE input at time $t$ \\
$\mathbf{A}_{\text{phys}}$ & Matrix & $\mathbb{R}^{N \times N}$ & Normalized physical adjacency \\
$\mathbf{A}_{\text{adp}}$ & Matrix & $\mathbb{R}^{N \times N}$ & Learnable adaptive adjacency \\
$\mathbf{A}$ & Matrix & $\mathbb{R}^{N \times N}$ & Combined adjacency matrix \\
$\alpha$ & Scalar & $[0,1]$ & Physical/adaptive mixing coefficient \\
$\kappa$ & Scalar & $\mathbb{R}^{+}$ & Negative Binomial dispersion parameter \\
$\lambda$ & Scalar & $\mathbb{R}^{+}$ & Auxiliary MSE loss weight ($\lambda = 0.3$) \\
$\mu_{h,i}$ & Scalar & $\mathbb{R}^{+}$ & Predicted mean for horizon $h$, station $i$ \\
$d_{\text{model}}$ & Scalar & $1$ & Model hidden dimension ($d_{\text{model}} = 192$) \\
$d_h$ & Scalar & $1$ & Attention head dimension ($d_h = 32$) \\
$n_h$ & Scalar & $1$ & Number of attention heads ($n_h = 6$) \\
$K$ & Scalar & $1$ & Number of graph propagation hops ($K = 3$) \\
$r$ & Scalar & $1$ & LoRA rank ($r = 16$) \\
$\alpha_{\text{LoRA}}$ & Scalar & $1$ & LoRA scaling factor ($\alpha_{\text{LoRA}} = 16$) \\
$\mathbf{W}_0$ & Matrix & $\mathbb{R}^{d_{\text{model}} \times F}$ & Frozen base weight matrix (LoRA) \\
$\mathbf{A}$ (LoRA) & Matrix & $\mathbb{R}^{r \times F}$ & LoRA down-projection \\
$\mathbf{B}$ & Matrix & $\mathbb{R}^{d_{\text{model}} \times r}$ & LoRA up-projection \\
$\mathbf{U}'$ & Tensor & $\mathbb{R}^{(BN) \times T \times d_{\text{model}}}$ & Reshaped GRE outputs for attention \\
$\mathbf{h}_{\text{temp}}$ & Vector & $\mathbb{R}^{d_{\text{model}}}$ & Mean-pooled temporal station representation \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Open Real-World Benchmark Dataset}
\label{sec:open_real_dataset}
To demonstrate generalisation beyond the synthetic Astana network, DTS-GSSF is also evaluated on a publicly available open dataset from the Los Angeles County Metropolitan Transportation Authority (LACMTA) Open Data Portal. The benchmark contains hourly boarding counts for 180 bus stops across nine service routes, together with GTFS-derived stop metadata, route connectivity, and schedule information. The dataset is fully open-source under the LACMTA terms and provides a realistic testbed for cross-city validation.

The open dataset was preprocessed with the same pipeline used for the synthetic data. Station-hour features were constructed from boardings and alightings; cyclical temporal encodings were added; weather covariates were merged from the publicly available NOAA archive; and the network graph was derived from GTFS stop adjacency. After train/validation/test splitting, the open benchmark contains 62,304 hourly observation windows across 180 stations and 9 routes. This dataset enables evaluation of the same DTS-GSSF model on a different urban transit system while preserving the graph-based spatiotemporal forecasting formulation.
...
\textbf{Cyclical and lag (8 features):} hour\_sin, hour\_cos, dow\_sin, dow\_cos, roll\_6h, roll\_24h, dev\_24h, and ratio\_24h. All input features are z-score normalised using training-set statistics only to prevent data leakage; target counts remain in their original scale as required by the Negative Binomial likelihood.
...
\textbf{Relation to state-space models.} While the GRE shares the sequential state-update structure of state-space models (SSMs), it differs from structured SSMs such as S4 \cite{gu2022efficiently} and Mamba \cite{gu2023mamba} in two key respects. First, the GRE uses a single multiplicative forget gate $\mathbf{a}_t$ rather than a diagonally structured state matrix $\mathbf{A}$ parameterised via HiPPO initialisation. Second, the GRE processes the sequence recurrently rather than via parallel scan, which precludes the $O(T \log T)$ training speedup of convolution-mode SSMs. This simpler formulation is adopted because (i) the moderate context length ($T = 72$) makes recurrent processing efficient, (ii) the single-gate design empirically exhibited stable gradient propagation over 72 steps in training runs---an empirical observation, not a theoretical guarantee; a rigorous gradient-norm comparison with stacked LSTM/GRU baselines is left to future work, and (iii) the per-step gating enables input-dependent forgetting, which is essential for the holiday and event effects in the ridership data under study.

The input projection uses LoRA (Low-Rank Adaptation) \cite{hu2022lora}:
\begin{equation}
    \mathbf{W}_{\text{in}}(\mathbf{x}) = \mathbf{W}_0 \mathbf{x} + \frac{\alpha}{r} \mathbf{B} \mathbf{A} \mathbf{x},
    \label{eq:lora}
\end{equation}
where $\mathbf{W}_0 \in \mathbb{R}^{d_{\text{model}} \times F}$ is the frozen base weight matrix, $\mathbf{A} \in \mathbb{R}^{r \times F}$ and $\mathbf{B} \in \mathbb{R}^{d_{\text{model}} \times r}$ are the low-rank decomposition matrices, $r = 16$ is the LoRA rank, and $\alpha = 16$ is the scaling factor. Only $\mathbf{A}$ and $\mathbf{B}$ are updated during route-specific fine-tuning, reducing the number of trainable parameters by 94\%.
...
\begin{equation}
    ...
    \label{eq:combined}
\end{equation}
where $\alpha = \sigma(\log \alpha_0)$ is a learnable mixing coefficient initialised at $0.6$ and constrained to $[0, 1]$ via the sigmoid function. The learnable $\alpha$ allows the model to balance physical topology and adaptive correlations during training; the learned value is reported in Section~\ref{sec:ablation}.
...
While the Gated Recurrent Encoder captures local temporal dynamics, the TemporalAttention module models long-range dependencies across the full $T = 72$ hour context. The GRE output tensor $\mathbf{U} \in \mathbb{R}^{B \times N \times T \times d_{\text{model}}}$ is reshaped to $\mathbf{U}' \in \mathbb{R}^{(BN) \times T \times d_{\text{model}}}$ so that multi-head self-attention operates independently per station. We apply attention with $n_h = 6$ heads and head dimension $d_h = 32$ over the $T$ timesteps for each station.
...
\begin{equation}
    ...
    \label{eq:attention}
\end{equation}
where $\mathbf{Q}_h = \mathbf{U}' \mathbf{W}_h^Q$, $\mathbf{K}_h = \mathbf{U}' \mathbf{W}_h^K$, $\mathbf{V}_h = \mathbf{U}' \mathbf{W}_h^V$, and the projection matrices $\mathbf{W}_h^Q, \mathbf{W}_h^K, \mathbf{W}_h^V \in \mathbb{R}^{d_{\text{model}} \times d_h}$ are shared across all stations. The outputs of all heads are concatenated and projected back to $d_{\text{model}}$. A residual connection and layer normalization \cite{ba2016layernorm} are applied. The attended outputs are averaged across time steps via mean pooling to produce a fixed-size station representation $\mathbf{h}_{\text{temp}} \in \mathbb{R}^{d_{\text{model}}}$ per station.
...
Two prediction heads produce the final forecasts. The \textbf{per-station head} is a 3-layer MLP ($192 \to 384 \to 192 \to 4$) with GELU activations between hidden layers, dropout ($p = 0.1$) applied after the first two layers, and a linear output layer producing $\mu_{h,i}$ for each of the $H_{\text{pred}} = 4$ horizons. The \textbf{aggregate head} uses a LoRA-adapted linear projection that maps the mean-pooled station representation from $d_{\text{model}} = 192$ to $12$ outputs (3 aggregate statistics per horizon), with only the low-rank matrices $\mathbf{A}$ and $\mathbf{B}$ updated during fine-tuning.
...
\begin{itemize}
    ...
    \item Physical graph propagation: $O(K \cdot |\mathcal{E}| \cdot d)$ where $|\mathcal{E}| = 1{,}142$ is the number of directed edges --- sparse in station count.
    \item Adaptive adjacency construction: $O(N^2 \cdot d_{\text{emb}})$ --- dense quadratic in station count.
    \item TemporalAttention: $O(N \cdot T^2 \cdot d)$ --- quadratic in context length.
    \item Prediction Heads: $O(N \cdot d)$ --- linear.
\end{itemize}
The total per-step complexity is $O(N \cdot T \cdot d + K \cdot |\mathcal{E}| \cdot d + N^2 \cdot d_{\text{emb}} + N \cdot T^2 \cdot d)$. For the configuration adopted ($N = 374$, $|\mathcal{E}| = 1{,}142$, $T = 72$, $K = 3$, $d = 192$, $d_{\text{emb}} = 16$), the dominant term is TemporalAttention at $O(N \cdot T^2 \cdot d) \approx 3.7 \times 10^8$ operations per batch, while physical graph propagation contributes $O(K \cdot |\mathcal{E}| \cdot d) \approx 6.6 \times 10^5$ operations and adaptive adjacency construction contributes $O(N^2 \cdot d_{\text{emb}}) \approx 2.2 \times 10^6$ operations. The sparse physical propagation is two orders of magnitude cheaper than a dense $O(K \cdot N^2 \cdot d)$ alternative, which motivates the dual-adjacency design. Although attention scales quadratically in context length, $T = 72$ keeps this term tractable on consumer-grade GPUs. For applications requiring longer contexts ($T > 200$), a linear-attention variant could replace the softmax mechanism while preserving the dual-timescale inductive bias.

\section{System Interface and Workflow}
\label{sec:workflow}

The complete system comprises three pipelines: (i)~data generation, which parses OSM data and simulates hourly ridership; (ii)~model training, using the configuration in Table~\ref{Tab.hyperparams}; and (iii)~inference deployment, where the trained model is served via a FastAPI backend with WebSocket-based alerts for anomalous demand events. A React-based dashboard provides real-time visualisation of predictions, per-station confidence intervals, and LoRA-based route-specific fine-tuning triggers. The system interfaces are presented in Appendix~\ref{sec:ui-screenshots}.
...
\subsection{Methodological Limitations of the Architecture}
\label{sec:method_limitations}

Several deliberate simplifications in the present design warrant explicit acknowledgment:

\textbf{Global dispersion parameter.} The Negative Binomial likelihood uses a single scalar $\kappa$ shared across all $N = 374$ stations and $H_{\text{pred}} = 4$ horizons. This assumes homogeneous overdispersion across the network, which is a strong and likely incorrect assumption: central business districts (e.g., Esil) are expected to exhibit different variance profiles from peripheral residential stops (e.g., Baikonur). A station-specific or district-specific $\kappa$ would add flexibility at modest parameter cost and is left to future work.

\textbf{Sinusoidal input--generator overlap.} The model receives $\texttt{hour\_sin}, \texttt{hour\_cos}, \texttt{dow\_sin}, \texttt{dow\_cos}$ as direct input features, and the synthetic generator (Eq.~\ref{eq:base_demand}) uses the same sinusoidal basis functions $f_{\text{hour}}, f_{\text{dow}}$ to construct the base demand. The model therefore has direct access to the exact temporal basis used to generate the signal. While the residual learning task (predicting ridership given weather, holiday, event, and log-normal noise multipliers) remains non-trivial, this overlap likely inflates the achievable R$^2$ on the synthetic benchmark relative to what a non-parametric temporal generator would yield. A future ablation regenerating the synthetic dataset without explicit cyclical features would quantify this effect.

\textbf{Potentially dense adaptive adjacency.} The adaptive adjacency $\mathbf{A}_{\text{adp}}$ can in principle assign non-negligible weight to geographically distant stations. A locality-bias regularizer or a top-$k$ sparsification would prevent unrealistic long-range edges; this extension is left to future work.

\textbf{Static graph topology.} The model assumes a fixed physical adjacency $\mathbf{A}_{\text{phys}}$ derived from the route network. In operational transit, routes are periodically adjusted due to construction, policy changes, and special events; a dynamic graph formulation is left to future work (see Sec.~\ref{sec:future}).
