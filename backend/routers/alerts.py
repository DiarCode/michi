...
from backend.services.alert_service import add_alert_rule, delete_rule, generate_auto_alerts, get_alert_rules, list_alerts
...
def trigger_auto_alerts(db: Session = Depends(get_db_session)):
    ...
    routes = [{"id": r.id, "name": r.name, "avg_ridership": r.avg_ridership}
