...
def load_seed_data():
    ...
    if SEED_PATH.exists():
        with open(SEED_PATH, encoding="utf-8") as f:
            data = json.load(f)
        logger.info(
            "Loaded OSM data: %d stations, %d routes",
            data["metadata"]["total_stations"],
            data["metadata"]["total_routes"],
        )
    ...
    try:
        ...
        for rs in route_stops:
            if isinstance(rs, dict):
                session.add(
                    RouteStopORM(route_id=rs["route_id"], station_id=rs["station_id"], stop_order=rs["stop_order"])
                )
            else:
        ...
        session.commit()
        logger.info(
            "Seeded %d stations, %d routes, %d route stops and dashboard data.",
            len(stations),
            len(routes),
            len(route_stops),
        )
