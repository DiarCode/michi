...
function TrendArrow({ current, previous }: { current: number; previous: number | undefined }) {
  if (previous === undefined) return <HugeiconsIcon icon={MinusSignIcon} strokeWidth={1.5} className="h-3 w-3 text-gray-400" />;
  if (current < previous) return <HugeiconsIcon icon={ArrowDownRight01Icon} strokeWidth={1.5} className="h-3 w-3 text-green-500" />;
  if (current > previous) return <HugeiconsIcon icon={ArrowUpRight01Icon} strokeWidth={1.5} className="h-3 w-3 text-red-500" />;
  return <HugeiconsIcon icon={MinusSignIcon} strokeWidth={1.5} className="h-3 w-3 text-gray-400" />;
}
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-1">
              ...
              <HugeiconsIcon icon={Activity01Icon} strokeWidth={1.5} className="h-3.5 w-3.5 text-gray-400" />
            </div>
            <div className="flex items-center gap-2">
              ...
              {driftAlerts.length > 0 && (
                <span className="text-xs text-amber-600 flex items-center gap-1">
                  <HugeiconsIcon icon={Alert02Icon} strokeWidth={1.5} className="h-3 w-3" />
                  {driftAlerts.length}
                </span>
              )}
            </div>
